...
export class CodegenState {
    ...
    constructor(options) {
        ...
        this.transforms = []
        this.dataGenFns = [genDataClass, genDataStyle]
        this.directives = extend(extend({}, baseDirectives), options.directives)
        const isReservedTag = options.isReservedTag || no
        this.maybeComponent = (el) => !(isReservedTag(el.tag) && !el.component)
        this.onceId = 0
...
export function genFor(el, state, altGen,altHelper) {
    ...
    if (
        ...
        state.warn(
            `<${el.tag} v-for="${alias} in ${exp}">: 使用 v-for指令，最好配套使用 key`
        )
    }
...
function genForScopedSlot(key, e, state) {
    const exp = el.for
    const alias = el.alias
    const iterator1 = el.iterator1 ? `,${el.iterator1}` : ''
    const iterator2 = el.iterator2 ? `,${el.iterator2}` : ''
